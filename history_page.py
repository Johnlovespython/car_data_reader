...
class HistoryPage(QWidget):
    ...
    def initUI(self):
            ...
            # QHBoxLayout-ის ნაცვლად ვიყენებთ QSplitter-ს მარცხენა და მარჯვენა მხარისთვის
           ...
           # ღილაკების ჰორიზონტალური ბლოკი
            ...
            """)
            self.delete_all_btn.clicked.connect(self.delete_all_logs) # ან შენი ფუნქციის სახელი
            ...
            # 2. სამივე ღილაკის დამატება layout-ში:
            list_buttons_layout.addWidget(self.refresh_btn)
            list_buttons_layout.addWidget(self.delete_btn)
            list_buttons_layout.addWidget(self.delete_all_btn) # <--- აი ეს დაამატე!
            ...
            # --- CONTROLS BAR (Play, Pause, Step, Stop) ---
            ...
            # ვამატებთ მარცხენა და მარჯვენა პანელებს QSplitter-ში
            main_splitter.addWidget(left_widget)
            main_splitter.addWidget(right_widget)
            
            # ვაყენებთ ფიქსირებულ საწყის ზომებს (მარცხენა: 280px, მარჯვენა: დანარჩენი)
    ...
    def delete_all_logs(self):
            # 1. დადასტურების დიალოგური ფანჯარა
            ...
            if confirm == QMessageBox.Yes:
                try:
                    # 2. რეპლეის გაჩერება და მეხსიერების გასუფთავება
                    self.stop_replay()
                    import gc
                    gc.collect()

                    # 3. ლოგების საქაღალდის გზის მიღება
                    logs_dir = self.get_logs_directory() if hasattr(self, 'get_logs_directory') else "logs"
                    ...
                    # 4. საქაღალდეში არსებული ყველა CSV ფაილის წაშლა
                    ...
                    # 5. UI-ს და ინტერფეისის სრული გასუფთავება
                    self.replay_data = []
                    self.signal_history = {}
                    self.signal_combo.clear()
                    self.replay_table.setRowCount(0)
                    ...
                    # 6. სიის განახლება და სტატუსის გამოტანა
                    self.load_log_files()
    ...
    def delete_selected_log(self):
        ...
        if confirm == QMessageBox.Yes:
            try:
                # გაჩერდეს რეპლეი, თუ ამ დროს მიმდინარეობს
                self.stop_replay()

                # ფაილის წაშლა დისკიდან
                if os.path.exists(filepath):
                    os.remove(filepath)

                # მონაცემებისა და ინტერფეისის გასუფთავება
                self.replay_data = []
                self.signal_history = {}
                self.signal_combo.clear()
                self.replay_table.setRowCount(0)
    ...
    def render_state_at_index(self, target_index):
        """სრულად აახლებს ცხრილს და გრაფიკს მითითებულ კადრამდე (target_index)"""
        self.current_index = target_index
        
        # 1. Slider & Counter Update
